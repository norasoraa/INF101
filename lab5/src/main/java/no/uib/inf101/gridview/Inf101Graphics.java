...
public class Inf101Graphics {
	...
	/**
	 * Draw a string centered in the given shape. The font of the Graphics object
	 * will be scaled such that the string fits inside the shape. The string will be
	 * drawn centered inside the shape. The font size is reverted once the string
	 * has been drawn.
	 * 
	 * @param g     The graphics context to draw on
	 * @param s     The String to draw
	 * @param shape in whose bounding box the string will be scaled and centered
	 */
	public static void drawScaledCenteredString(Graphics g, String s, Shape shape) {
		FontMetrics fm = g.getFontMetrics();
		Font originalFont = g.getFont();
		double maxWidth = shape.getBounds2D().getWidth();
		double maxHeight = shape.getBounds2D().getHeight();
		double scaleX = maxWidth / fm.stringWidth(s);
		double scaleY = maxHeight / fm.getHeight();
		double scale = Math.min(scaleX, scaleY);
		float fontSize = (float) (originalFont.getSize() * scale);
		Font scaledFont = originalFont.deriveFont(fontSize);
		g.setFont(scaledFont);
		drawCenteredString(g, s, shape);
